...
const peli = (infopeli) => {
  ...
  valoracion.innerHTML = `Valoración: ⭐<b>${infopeli.vote_average}</b> (votos: ${infopeli.vote_count})<br>
  Géneros: ${mostrartodo(infopeli.genres, 'name')}<br>Fecha de estreno: ${infopeli.release_date}<br>Estado: ${infopeli.status}<br>
  ...
  masinfo.innerHTML = `Idioma original: ${infopeli.original_language} <br>Presupuesto: $${infopeli.budget}
  <br>Ingresos: $${infopeli.revenue} <br>Productoras: ${mostrartodo(infopeli.production_companies, 'name')}<br>Paises: ${mostrartodo(infopeli.production_countries, 'name')}`;
  //se crea la galería multimedia
  fetch(`${BASE_URL}/movie/${ident}/videos?api_key=${API_KEY}&language=es-ES&page=1`)
    .then((res) => res.json())
    .then((infovideo) => {
      videos(infovideo);
    });
...
document.addEventListener("DOMContentLoaded", () => {
  fetch(`${BASE_URL}/movie/${ident}?api_key=${API_KEY}&language=es-ES&page=1`)
    .then((res) => res.json())
    .then((infopeli) => {
      peli(infopeli);
    });
