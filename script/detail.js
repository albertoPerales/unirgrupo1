...
const peli = (infopeli) => {
  ...
  valoracion.innerHTML = `Valoración: ⭐<b>${infopeli.vote_average}</b> (votos: ${infopeli.vote_count})`;
  info.append(movieinfo);
  info.append(valoracion);
  //se crea la información extra
  masinfo.innerHTML = `Idioma original: ${infopeli.original_language}<br>Fecha de estreno: ${infopeli.release_date}`;
};
